import data
import logic
import config


def generate_weekly_summary(id_class):
    t = data.team.read_mainfile(id_class)
    t6 = []
    for i in t["idteam"]:
        if data.team.check_team(i["id_team"]):
            t2 = data.summary.read_main("week")
            t3 = []
            t8 = data.summary.read_main("semester")
            if t8["num"] == 0:
                for h in range(t2["num"]):
                    t5 = []
                    t4 = data.summary.read(i["id_team"], "week", h + 1)
                    if t4 and isinstance(t4, dict):
                        for j in t4.values():
                            if isinstance(j, dict):
                                t5.append([j["name"], j["total"], j["ratings"]])
                    t3.append(t5)
            elif t8["num"] == 1:
                for h in range(config.semester_1, t2["num"]):
                    t5 = []
                    t4 = data.summary.read(i["id_team"], "week", h + 1)
                    if t4 and isinstance(t4, dict):
                        for j in t4.values():
                            if isinstance(j, dict):
                                t5.append([j["name"], j["total"], j["ratings"]])
                    t3.append(t5)
            t6.append([int(i["id_team"]), t3])
            t3 = []

    data.summary.create("semester", id_class)
    t7 = []
    for i in range(len(t6)):
        t7.append([t6[i][0], []])
        for j in range(1, len(t6[i])):
            for h in range(len(t6[i][j])):
                for k in range(len(t6[i][j][h])):
                    student_name = t6[i][j][h][k][0]
                    student_total = t6[i][j][h][k][1]
                    rating = t6[i][j][h][k][2]
                    for l in range(len(t7[-1][1])):
                        if t7[-1][1][l][0] == student_name:
                            break
                    else:
                        t7[-1][1].append([student_name, 0, [
                            [0, config.not_achieved], [0, config.achieved],
                            [0, config.medium], [0, config.rather],
                            [0, config.good], [0, config.very_good],
                            [0, config.super_good]
                        ]])
                        l = len(t7[-1][1]) - 1
                    t7[-1][1][l][1] += student_total
                    if rating == config.not_achieved:
                        t7[-1][1][l][2][0][0] += 1
                    elif rating == config.achieved:
                        t7[-1][1][l][2][1][0] += 1
                    elif rating == config.medium:
                        t7[-1][1][l][2][2][0] += 1
                    elif rating == config.rather:
                        t7[-1][1][l][2][3][0] += 1
                    elif rating == config.good:
                        t7[-1][1][l][2][4][0] += 1
                    elif rating == config.very_good:
                        t7[-1][1][l][2][5][0] += 1
                    elif rating == config.super_good:
                        t7[-1][1][l][2][6][0] += 1

    for i in range(len(t7)):
        for j in range(len(t7[i][1])):
            t7[i][1][j][2].sort(reverse=True)
            t7[i][1][j][2] = t7[i][1][j][2][0][1]

    for i in t7:
        data.summary.write(i[0], {"students": i[1]}, "semester", id_class)

    return t7
